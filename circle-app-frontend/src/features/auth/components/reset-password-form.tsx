...
export default function ResetPasswordForm() {
  ...
  const { register, handleSubmit, errors, isSubmitting, onResetPassword } = useResetPassword(tokenData);
    ...
    <Box color={"white"} width={"brand.form"}>
      ...
      <form onSubmit={handleSubmit(onResetPassword)}>
        <FormControl display={"flex"} flexDirection={"column"} gap={"12px"}>
          <Input {...register("password")} type="password" placeholder="New Password*" rounded={8} padding={5} fontSize={14} fontWeight={500} borderColor={"brand.borderAbu"} />
          <Input {...register("newPassword")} type="password" placeholder="Confirm New Password*" rounded={8} padding={5} fontSize={14} fontWeight={500} borderColor={"brand.borderAbu"} />
          {errors.newPassword && (
