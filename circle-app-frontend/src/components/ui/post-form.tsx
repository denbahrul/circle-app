...
export default function FormPost({ placeholder, buttonTitle }: { placeholder: string; buttonTitle: string }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <FormControl display={"flex"} alignItems={image ? "start" : "center"} gap={4} justifyContent={"space-between"} borderBottom={"solid 1px"} borderColor={"brand.borderAbu"} p={4}>
        ...
        <Box flex={"1"}>
          ...
          <Input {...register("image")} onChange={(e) => onImageChage(e, register("image").onChange)} id="upload" type="file" variant={"unstyled"} border={"none"} hidden />
