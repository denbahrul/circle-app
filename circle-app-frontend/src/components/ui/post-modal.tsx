import { Box, Button, Flex, Image, Input, Text, Textarea, FormControl } from "@chakra-ui/react";
    ...
    <ModalContent maxW={"740px"} backgroundColor={"brand.backgroundCircle"} rounded={15}>
      ...
      <ModalBody mt={10} px={6} pb={4} pt={0}>
        <FormControl display={"flex"} alignItems={"flex-start"} gap={4} justifyContent={"space-between"}>
          ...
          <Box flex={"1"}>
            <Textarea variant={"unstyled"} border={"none"} placeholder="What is happening?!" />
          </Box>
        </FormControl>
      </ModalBody>
      <Box p={4} display={"flex"} alignItems={"center"} justifyContent={"space-between"} gap={4} px={6} borderTop={"solid 1px"} borderColor={"brand.borderAbu"}>
        ...
        <Button backgroundColor={"brand.green-dark"} color={"brand.white-dark"} height={"33px"} justifyItems={"center"} rounded={"full"} alignItems={"center"} padding={4} fontSize={"14px"} fontWeight={700} lineHeight={"17px"}>
          Post
        </Button>
      </Box>
    </ModalContent>
