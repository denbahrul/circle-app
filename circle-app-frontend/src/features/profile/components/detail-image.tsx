...
import FormReply from "../../detail/componenets/reply-form";
import RepliesItem from "../../detail/componenets/replies-item";
    ...
    <Flex justifyContent={"space-between"} p={2}>
      <Box flex={"1"} position={"sticky"} top={0} px={2} height={"100vh"}>
        <Box position={"absolute"} top={4} left={2}>
          <HiXCircle size={34} onClick={() => navigate(-1)} cursor={"pointer"} />
        </Box>
        <Box onClick={isContentOpen ? fullImage : openContent} cursor={"pointer"} rounded={"full"} border={"solid 2px"} borderColor={"white"} position={"absolute"} top={4} right={2}>
          {isContentOpen ? <HiChevronRight size={24} /> : <HiChevronLeft size={24} />}
        </Box>
        <Image src={threads.image} height={"100%"} width={"100%"} rounded={4} objectFit="contain" />
      </Box>
      {isContentOpen && (
